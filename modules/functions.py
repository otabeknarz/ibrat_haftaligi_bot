    ...
    if response.ok:
        ...
        if invited_user_response.ok:
            ...
            if invited_user_username.isdigit():
                ...
                await bot.send_message(
                    invited_by_id, f"🥳 Tabriklaymiz! Siz <strong>{invited_user_name}</strong> ni taklif qildingiz."
                )
            else:
                await bot.send_message(
                    invited_by_id,
                    f"🥳 Tabriklaymiz! Siz <strong>@{invited_user_username}</strong> ni taklif qildingiz.",
                )
        ...
        if invited_by_response.ok:
            ...
            if (
                ...
                await bot.send_message(
                    invited_by_id,
                    """<strong>🙌🏻 Barakalla, siz buni uddaladingiz!</strong>

🤩 Siz shartni to'liq bajarib, 7 ta do'stingizni loyihaga taklif qilib <strong>"Ibrat haftaligi – 7 kun, 7 yangi marra"</strong> ning rasman qatnashchisiga aylandingiz!
